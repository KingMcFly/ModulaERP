...
router.get('/', guard, w(async (req, res) => {
  ...
  let sql = 'SELECT * FROM providers WHERE tenant_id = ? AND is_active = true';
...
router.post('/', guard, w(async (req, res) => {
  ...
  const [rows] = await db.query(
    'INSERT INTO providers (tenant_id, name, rut, contact_name, email, phone, address, category, notes) VALUES (?,?,?,?,?,?,?,?,?) RETURNING id',
...
router.delete('/:id', guard, w(async (req, res) => {
  await db.query('UPDATE providers SET is_active=false WHERE id=? AND tenant_id=?', [req.params.id, req.user.tenant_id]);
