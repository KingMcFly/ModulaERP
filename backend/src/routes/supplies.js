...
router.get('/low-stock', guard, w(async (req, res) => {
  const [rows] = await db.query(
    'SELECT * FROM supplies WHERE tenant_id = ? AND is_active = true AND current_stock <= min_stock ORDER BY name',
    [req.user.tenant_id]
  );
  res.json(rows);
}));
...
router.get('/', guard, w(async (req, res) => {
  const [rows] = await db.query(
    `SELECT s.*, l.name AS location_name FROM supplies s
     ...
     WHERE s.tenant_id = ? AND s.is_active = true ORDER BY s.name`,
    [req.user.tenant_id]
  );
  res.json(rows);
}));
...
router.delete('/:id', guard, w(async (req, res) => {
  await db.query('UPDATE supplies SET is_active = false WHERE id = ? AND tenant_id = ?',
    [req.params.id, req.user.tenant_id]);
