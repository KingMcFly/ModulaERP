...
router.get('/', guard, w(async (req, res) => {
  const [rows] = await db.query(
    `SELECT cc.*,
     ...
     FROM cost_centers cc WHERE cc.tenant_id=? AND cc.is_active=true ORDER BY cc.name`,
    [req.user.tenant_id]
  );
  res.json(rows);
}));
...
router.post('/', guard, w(async (req, res) => {
  ...
  const [rows] = await db.query(
    'INSERT INTO cost_centers (tenant_id, name, code, description, manager, budget) VALUES (?,?,?,?,?,?) RETURNING id',
...
router.delete('/:id', guard, w(async (req, res) => {
  await db.query('UPDATE cost_centers SET is_active=false WHERE id=? AND tenant_id=?', [req.params.id, req.user.tenant_id]);
